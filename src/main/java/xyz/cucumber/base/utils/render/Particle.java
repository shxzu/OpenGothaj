...
public class Particle {
    ...
    public boolean draw() {
        ...
        if (this.time < (float)System.nanoTime() / 1000000.0f || this.x < 0.0 || this.y < 0.0 || this.x > (double)sr.getScaledWidth() || this.y > (double)sr.getScaledHeight()) {
            return true;
        }
        ...
        this.update();
        RenderUtils.drawPoint(this.x, this.y, finalc, this.size);
        return false;
    }

    public void update() {
        this.x += Math.sin(Math.toRadians(this.yaw)) * this.speed;
        this.y += Math.cos(Math.toRadians(this.yaw)) * this.speed;
    }
