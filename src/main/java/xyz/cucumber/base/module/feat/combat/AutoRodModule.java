...
@ModuleInfo(category=Category.COMBAT, description="Automatically throw projectiles into player", name="Auto Rod", key=0, priority=ArrayPriority.HIGH)
...
extends Mod {
    ...
    public void onRotation(EventRotation event) {
        ...
        this.target = EntityUtils.getTarget(10.0, "Players", "Off", 400, Client.INSTANCE.getModuleManager().getModule(TeamsModule.class).isEnabled(), this.ka.TroughWalls.isEnabled(), this.ka.attackInvisible.isEnabled(), this.ka.attackDead.isEnabled());
        if (this.target == null) {
            if (this.cancel) {
                this.cancel = false;
                RotationUtils.customRots = false;
        ...
        if (!this.ka.isEnabled() || EntityUtils.getDistanceToEntityBox(this.target) < Math.max(this.ka.Range.value, 4.0) || this.target.hurtTime >= 3) {
            if (this.cancel) {
                this.cancel = false;
                RotationUtils.customRots = false;
            }
        ...
        if (!this.timer.hasTimeElapsed(delay, false)) {
            if (this.cancel) {
                this.cancel = false;
                RotationUtils.customRots = false;
            }
