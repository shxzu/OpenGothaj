...
@ModuleInfo(category=Category.PLAYER, description="Detects staff who joined", name="Staff Detector")
...
extends Mod {
    ...
    private String[] qplay = new String[]{"Profikk", "_Spetty_", "Ayessha", "ItzTadeas", "_razorRalfcz", "Dastrokk", "Linuuus", "KwenT_", "M1che_", "GodLikeKubiss", "SeptunLover", "Veverka14", "DeKoN_CZ", "H0t_Mamka", "Hrib", "Majulka", "DarkBanan", "iArthurr_", "matyraptycz", "stepanpanik", "Sunnyyyyyyy", "RuzovaTabletkaa", "dulisek231"};

    public StaffDetectorModule() {
        this.addSettings(this.mode, this.autoLobby, this.lobbyCommand);
    }
    ...
    public void onRecieve(EventReceivePacket e) {
        ...
        if (e.getPacket() instanceof S14PacketEntity) {
            ...
            if (!this.detected) {
                Entity player = this.mc.theWorld.getEntityByID(packet.getEntityId());
                if (player == null) {
                    Client.INSTANCE.getCommandManager().sendChatMessage("Â§cStaff detected: Â§7Unknown");
                    if (this.autoLobby.isEnabled()) {
                        this.mc.thePlayer.sendChatMessage(this.lobbyCommand.getString());
