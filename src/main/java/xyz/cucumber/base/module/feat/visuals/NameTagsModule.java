...
extends Mod {
    ...
    public NameTagsModule() {
        this.addSettings(this.health, this.bloom);
    }
    ...
    public void onBloom(EventBloom e) {
        if (this.bloom.isEnabled()) {
            ...
            for (Map.Entry<EntityPlayer, PositionUtils> entry : this.entities.entrySet()) {
                ...
                double w = Fonts.getFont("rb-r").getWidth(user.runningClient.equals("-") ? player.getName() : "[" + user.runningClient + "]" + " " + user.nickname + " [" + CLUtils.getClientRoleByID(user.role) + "]") + (this.health.isEnabled() ? Fonts.getFont("rb-r").getWidth(String.valueOf((int)player.getHealth()) + " ") : 0.0);
                RenderUtils.drawRoundedRectWithCorners(pos.getX() + pos.getWidth() / 2.0 - w / 2.0 - 1.0, pos.getY() - 10.0, pos.getX() + pos.getWidth() / 2.0 + w / 2.0 + 1.0, pos.getY() - 10.0 + 8.0, -16777216, 2.0, true, true, true, true);
            }
        }
    }
    ...
    public void onRender2D(EventRenderGui e) {
        for (Map.Entry<EntityPlayer, PositionUtils> entry : this.entities.entrySet()) {
            ...
            GL11.glTranslated((double)(pos.getX() + pos.getWidth() / 2.0), (double)(pos.getY() - 10.0), (double)0.0);
            double w = Fonts.getFont("rb-r").getWidth(user.runningClient.equals("-") ? player.getName() : "[" + user.runningClient + "]" + " " + user.nickname + " [" + CLUtils.getClientRoleByID(user.role) + "]") + (this.health.isEnabled() ? Fonts.getFont("rb-r").getWidth(String.valueOf((int)player.getHealth()) + " ") : 0.0);
            ...
            if (this.health.isEnabled()) {
                Fonts.getFont("rb-r").drawString(String.valueOf((int)player.getHealth()), w / 2.0 - Fonts.getFont("rb-r").getWidth(String.valueOf((int)player.getHealth())), 3.0, ColorUtils.mix(-65536, -16711936, player.getHealth() < 0.0f ? 0.0f : player.getHealth(), player.getMaxHealth()));
            }
            Fonts.getFont("rb-r").drawString(user.runningClient.equals("-") ? player.getName() : "§7[" + user.runningClient + '§' + "7]" + " " + '§' + "f" + user.nickname + " " + '§' + "7[" + CLUtils.getClientRoleByID(user.role) + '§' + "7]", -w / 2.0, 3.0, -1);
