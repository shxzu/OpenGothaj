...
@ModuleInfo(category=Category.COMBAT, description="Allows you to remove hit slowdown", name="No Hit Slow", key=0)
public class NoHitSlowModule
extends Mod {
    public BooleanSettings onlyHurtTime = new BooleanSettings("Only Hurt Time", true);

    public NoHitSlowModule() {
        this.addSettings(this.onlyHurtTime);
    }
    ...
    public void onHit(EventHit e) {
        ...
        if (this.mc.thePlayer.hurtTime <= 0 || (velocity.mode.getMode().equalsIgnoreCase("Vanilla") || velocity.mode.getMode().equalsIgnoreCase("Vulcan")) && velocity.horizontal.getValue() == 0.0 && velocity.vertical.getValue() == 0.0 && velocity.isEnabled()) {
            e.setCancelled(true);
        }
