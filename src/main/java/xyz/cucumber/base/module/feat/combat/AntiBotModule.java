...
@ModuleInfo(category=Category.COMBAT, description="Automatically removes anticheat bot", name="Anti Bot", key=0)
...
extends Mod {
    ...
    public AntiBotModule() {
        this.addSettings(this.hypixel, this.matrix);
    }

    @EventListener
    public void onTick(EventTick e) {
        for (Entity ent : this.mc.theWorld.loadedEntityList) {
            if (ent == this.mc.thePlayer || !(ent instanceof EntityPlayer)) continue;
            ...
            if (!ent.isInvisible() || !this.hypixel.isEnabled()) continue;
            this.mc.theWorld.removeEntity(ent);
        }
    }
}
