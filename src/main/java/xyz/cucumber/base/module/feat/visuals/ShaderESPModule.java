...
extends Mod {
    ...
    public ShaderESPModule() {
        this.addSettings(this.settings, this.sensitivity, this.mode, this.color);
    }
    ...
    public void renderEntities(float partialTicks) {
        for (Entity player : this.mc.theWorld.playerEntities) {
            if (this.mc.getRenderManager() == null || !(player instanceof EntityPlayer) || player == this.mc.thePlayer && this.mc.gameSettings.thirdPersonView == 0 || !RenderUtils.isInViewFrustrum(player)) continue;
