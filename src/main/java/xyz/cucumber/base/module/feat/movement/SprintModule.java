...
extends Mod {
    ...
    public void onMoveForward(EventMoveForward e) {
        ...
        if (!(this.mc.thePlayer.isCollidedHorizontally || this.mc.thePlayer.isSneaking() || !this.mc.gameSettings.keyBindForward.pressed || this.mc.thePlayer.isSprinting() || !this.mc.thePlayer.isUsingItem() || !Client.INSTANCE.getModuleManager().getModule(NoSlowModule.class).isEnabled() && this.mc.gameSettings.keyBindUseItem.pressed)) {
            this.mc.thePlayer.setSprinting(true);
        }
        KillAuraModule ka = (KillAuraModule)Client.INSTANCE.getModuleManager().getModule("Kill Aura");
        if (RotationUtils.customRots && ka.MoveFix.getMode().equalsIgnoreCase("Silent") && ka.isEnabled() && ka.allowedToWork && (double)Math.abs(MathHelper.wrapAngleTo180_float((float)Math.toDegrees(MovementUtils.getDirection(this.mc.thePlayer.rotationYaw))) - MathHelper.wrapAngleTo180_float(RotationUtils.serverYaw)) > 67.5) {
        ...
        if (RotationUtils.customRots && smoothRotation.isEnabled() && !scaffold.isEnabled() && smoothRotation.wasScaffold && (double)Math.abs(MathHelper.wrapAngleTo180_float((float)Math.toDegrees(MovementUtils.getDirection(this.mc.thePlayer.rotationYaw))) - MathHelper.wrapAngleTo180_float(RotationUtils.serverYaw)) > 67.5) {
            this.mc.gameSettings.keyBindSprint.pressed = false;
            this.mc.thePlayer.setSprinting(false);
        }
    }
