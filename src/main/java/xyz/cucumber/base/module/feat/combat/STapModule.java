...
@ModuleInfo(category=Category.COMBAT, description="Automatically goes backwards to dodge hits", name="STap", key=0, priority=ArrayPriority.HIGH)
public class STapModule
extends Mod {
    ...
    public STapModule() {
        this.addSettings(this.holdTime);
    }
    ...
    public void onMoveButton(EventMoveButton e) {
        if (this.mc.thePlayer.hurtTime == 0 && this.mc.thePlayer.onGround && !this.mc.gameSettings.keyBindJump.pressed && !this.timer1.hasTimeElapsed(this.holdTime.getValue(), false) && this.timer.hasTimeElapsed(500.0, false) && this.mc.objectMouseOver.entityHit != null && this.mc.objectMouseOver.entityHit.hurtResistantTime > 0) {
            e.forward = false;
            e.backward = true;
        }
    }
