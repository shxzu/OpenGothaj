...
implements Dragable {
    ...
    public ScoreboardModule() {
        this.addSettings(this.positionX, this.positionY, this.rounded, this.blur, this.bloom, this.bloomColor, this.backgroundColor, this.outline, this.outlineColor);
    }
    ...
    public void onBlur(EventBlur e) {
        ...
        ScoreObjective scoreObjective = objective = scoreobjective != null ? scoreobjective : scoreboard.getObjectiveInDisplaySlot(1);
        if (objective == null) {
            return;
        }
        ...
        for (Score score2 : collection) {
            ...
            if (!(width < Fonts.getFont("rb-r").getWidth(s1))) continue;
    ...
    public void onBloom(EventBloom e) {
        ...
        if (this.bloom.isEnabled()) {
            ...
            int color = ColorUtils.getColor(this.bloomColor, System.nanoTime() / 1000000L, 0.0, 5.0);
            if (this.rounded.isEnabled()) {
    ...
    public void onRenderGui(EventRenderGui e) {
        ...
        int height = collection.size() * 9 + 16;
        int color = ColorUtils.getAlphaColor(ColorUtils.getColor(this.backgroundColor, System.nanoTime() / 1000000L, 0.0, 5.0), this.backgroundColor.getAlpha());
        if (this.rounded.isEnabled()) {
            RenderUtils.drawRoundedRect(pos[0], pos[1], pos[0] + width, pos[1] + (double)height, color, 2.0f);
        } else {
            RenderUtils.drawRect(pos[0], pos[1], pos[0] + width, pos[1] + (double)height, color);
        ...
        if (this.outline.isEnabled()) {
            color = ColorUtils.getAlphaColor(ColorUtils.getColor(this.outlineColor, System.nanoTime() / 1000000L, 0.0, 5.0), this.outlineColor.getAlpha());
            if (this.rounded.isEnabled()) {
        ...
        Fonts.getFont("rb-r").drawString(objective.getDisplayName(), pos[0] + width / 2.0 - Fonts.getFont("rb-r").getWidth(objective.getDisplayName()) / 2.0, pos[1] + 4.0, -1);
        int j = 0;
