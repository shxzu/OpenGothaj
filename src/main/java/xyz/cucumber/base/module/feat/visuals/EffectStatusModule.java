...
public class EffectStatusModule
extends Mod
implements Dragable {
    ...
    public EffectStatusModule() {
        this.addSettings(this.positionX, this.positionY, this.textStyle, this.fonts, this.rounded, this.modColor, this.blur, this.b, this.bloomColor, this.backgroundMode, this.backgroundColor);
    }
    ...
    public void onBloom(EventBloom e) {
        if (this.b.isEnabled()) {
            ...
            RenderUtils.drawCustomRect(this.position.getX(), this.position.getY(), this.position.getX2(), this.position.getY2() - 1.0, ColorUtils.getColor(this.bloomColor, System.nanoTime() / 1000000L, 0.0, 5.0), 1.0f, this.rounded.isEnabled());
        }
    }
    ...
    public void onBlur(EventBlur e) {
        if (this.blur.isEnabled()) {
            ...
            RenderUtils.drawCustomRect(this.position.getX(), this.position.getY(), this.position.getX2(), this.position.getY2(), -16777216, 1.0f, this.rounded.isEnabled());
        }
    }
    ...
    public void onRenderGui(EventRenderGui e) {
        ...
        RenderUtils.drawCustomRect(this.position.getX(), this.position.getY(), this.position.getX2(), this.position.getY2(), -14342363, 1.0f, this.rounded.isEnabled());
        StencilUtils.bindReadStencilBuffer(1);
        int color = ColorUtils.getColor(this.backgroundColor, System.nanoTime() / 1000000L, 0.0, 5.0);
        if (this.backgroundMode.getMode().equalsIgnoreCase("Text")) {
            ColorSettings fixedColor = new ColorSettings(this.modColor.getMode(), this.modColor.getMode(), this.modColor.getMainColor(), this.modColor.getSecondaryColor(), this.backgroundColor.getAlpha());
            color = ColorUtils.getColor(fixedColor, System.nanoTime() / 1000000L, 0.0, 5.0);
        }
        if (this.backgroundMode.getMode().equalsIgnoreCase("Default")) {
            color = -299818719;
        }
        RenderUtils.drawCustomRect(this.position.getX(), this.position.getY(), this.position.getX2(), this.position.getY2(), color, 1.0f, this.rounded.isEnabled());
        if (this.mc.thePlayer.activePotionsMap.entrySet().size() != 0) {
            ...
            GL11.glVertex2d((double)this.position.getX(), (double)(this.position.getY() + 12.0));
            RenderUtils.color(-5592406);
            GL11.glVertex2d((double)(this.position.getX() + this.position.getWidth() / 2.0), (double)(this.position.getY() + 12.0));
            RenderUtils.color(0xAAAAAA);
            GL11.glVertex2d((double)this.position.getX2(), (double)(this.position.getY() + 12.0));
            GL11.glEnd();
            RenderUtils.stop2D();
            GL11.glPopMatrix();
        }
        this.position.setHeight(this.animation);
        Fonts.getFont(font).drawString("Effects", this.position.getX() + this.position.getWidth() / 2.0 - Fonts.getFont(font).getWidth("Effects") / 2.0, this.position.getY() + 6.0, ColorUtils.getColor(this.modColor, System.nanoTime() / 1000000L, 0.0, 5.0));
        int i = 0;
        for (Map.Entry potions : this.mc.thePlayer.activePotionsMap.entrySet()) {
            Potion potion = Potion.potionTypes[((PotionEffect)potions.getValue()).getPotionID()];
            String potionName = ((PotionEffect)potions.getValue()).getEffectName();
            ...
            GL11.glPushMatrix();
            Fonts.getFont(font).drawString(String.valueOf(potionName) + " " + (((PotionEffect)potions.getValue()).getAmplifier() + 1 == 1 ? "" : String.valueOf(((PotionEffect)potions.getValue()).getAmplifier() + 1)), this.position.getX() + 2.0, this.position.getY() + 16.0 + (double)(10 * i), -3355444);
            Fonts.getFont(font).drawString(Potion.getDurationString((PotionEffect)potions.getValue()), this.position.getX2() - Fonts.getFont(font).getWidth(Potion.getDurationString((PotionEffect)potions.getValue())), this.position.getY() + 16.0 + (double)(10 * i), -1);
            GL11.glPopMatrix();
            ++i;
        }
