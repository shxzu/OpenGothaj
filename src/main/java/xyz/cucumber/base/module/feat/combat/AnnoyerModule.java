...
@ModuleInfo(category=Category.COMBAT, description="Automatically blocks player with blocks", name="Annoyer", key=0)
public class AnnoyerModule
extends Mod {
    ...
    public void onRotation(EventRotation e) {
        ...
        this.target = EntityUtils.getTarget(5.0, "Players", "Off", 400, Client.INSTANCE.getModuleManager().getModule(TeamsModule.class).isEnabled(), this.ka.TroughWalls.isEnabled(), this.ka.attackInvisible.isEnabled(), this.ka.attackDead.isEnabled());
        if (this.target == null) {
            this.cancel = false;
            return;
        }
        if (EntityUtils.getDistanceToEntityBox(this.target) <= 3.0) {
            this.target = null;
            this.cancel = false;
            return;
        }
        ...
        if (!this.timer.hasTimeElapsed(this.delay.getValue(), false)) {
            if (this.cancel) {
                this.cancel = false;
                RotationUtils.customRots = false;
            }
