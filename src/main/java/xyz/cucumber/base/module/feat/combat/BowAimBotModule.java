...
@ModuleInfo(category=Category.COMBAT, description="Automatically aims at player with bow", name="Bow Aim Bot", key=0, priority=ArrayPriority.HIGH)
...
extends Mod {
    ...
    public void onUpdate(EventUpdate e) {
        this.target = EntityUtils.getTarget(80.0, this.ka.Targets.getMode(), "Off", 400, Client.INSTANCE.getModuleManager().getModule(TeamsModule.class).isEnabled(), this.ka.TroughWalls.isEnabled(), this.ka.attackInvisible.isEnabled(), this.ka.attackDead.isEnabled());
        ...
        if (this.mc.thePlayer.getHeldItem().getItem() == null) {
            this.target = null;
            if (this.cancel) {
                RotationUtils.customRots = false;
                this.cancel = false;
    ...
    public float[] getBowRotations(EntityLivingBase entity) {
        ...
        double posY = this.target.posY + (this.target.posY - this.target.prevPosY) * 1.0 + (double)this.target.height * 0.5 - this.mc.thePlayer.posY - (double)this.mc.thePlayer.getEyeHeight();
        ...
        if (Float.isNaN(neededPitch)) {
            return RotationUtils.getInstantTargetRotation(entity, entity.posX, entity.posY, entity.posZ, false);
        }
        pitch = neededPitch;
