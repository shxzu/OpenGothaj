...
public abstract class Panel {
    ...
    public void draw(int mouseX, int mouseY) {
        this.animation = this.position.isInside(mouseX, mouseY) ? (this.animation * 14 + 80) / 15 : (this.animation * 14 + 40) / 15;
        ...
        Fonts.getFont("rb-r-13").drawString(ConfigFileUtils.getTimeDifference(this.time), this.position.getX() + 5.0 + Fonts.getFont("rb-m-13").getWidth("@" + this.creator) + 5.0, this.position.getY() + 14.0, -5592406);
        ...
        if (this.informations.isInside(mouseX, mouseY) && this.description != "") {
            RenderUtils.drawRoundedRect(this.informations.getX() + this.informations.getWidth() / 2.0 - 50.0, this.informations.getY() - 70.0, this.informations.getX() + this.informations.getWidth() / 2.0 + 50.0, this.informations.getY(), -15724528, 2.0f);
            RenderUtils.drawOutlinedRoundedRect(this.informations.getX() + this.informations.getWidth() / 2.0 - 50.0, this.informations.getY() - 70.0, this.informations.getX() + this.informations.getWidth() / 2.0 + 50.0, this.informations.getY(), 0x20777777, 2.0, 1.0);
            String[] descSplitted = this.description.split(" ");
            String finalStr = "";
            int y = 0;
            int i = 0;
            while (i < descSplitted.length) {
                if (Fonts.getFont("rb-r-13").getWidth(finalStr) + Fonts.getFont("rb-r-13").getWidth(descSplitted[i]) < 94.0) {
                    finalStr = String.valueOf(finalStr) + (i == 0 ? "" : " ") + descSplitted[i];
                } else if (Fonts.getFont("rb-r-13").getWidth(descSplitted[i]) > 94.0) {
                    String[] sp;
                    String[] stringArray = sp = descSplitted[0].split("");
                    int n = sp.length;
                    int n2 = 0;
                    while (n2 < n) {
                        String s = stringArray[n2];
                        if (Fonts.getFont("rb-r-13").getWidth(finalStr) + Fonts.getFont("rb-r-13").getWidth(s) < 94.0) {
                            finalStr = String.valueOf(finalStr) + s;
                        } else {
                            Fonts.getFont("rb-r-13").drawString(finalStr, this.informations.getX() + this.informations.getWidth() / 2.0 - 47.0, this.informations.getY() - 65.0 + (double)(10 * y), -8947849);
                            finalStr = s;
                            ++y;
                        }
                        ++n2;
