...
public class ConfigManager {
    ...
    public void draw(int mouseX, int mouseY) {
        ...
        RenderUtils.drawRoundedRectWithCorners(this.openButton.getX(), this.openButton.getY(), this.openButton.getX2() + 1.0, this.openButton.getY2(), -14671840, 3.0, true, false, true, false);
        RenderUtils.drawImage(this.openButton.getX() + 1.0, this.openButton.getY() + 3.0, this.openButton.getWidth(), this.openButton.getWidth(), new ResourceLocation("client/images/gothaj.png"), -1);
        ...
        int i = 0;
        RenderUtils.drawRoundedRect(this.position.getX() + this.position.getWidth() / 2.0 - 75.0, this.position.getY() + 35.0, this.position.getX() + this.position.getWidth() / 2.0 + 75.0, this.position.getY() + 48.0, -15198184, 3.0f);
        ...
        for (Panel panel : this.active.configs) {
            if (!panel.getName().toLowerCase().contains(this.search.getText().toLowerCase()) || !panel.getCreator().toLowerCase().contains(this.search.getText().toLowerCase())) continue;
    ...
    public void renderTextField(GuiTextField field) {
        ...
        if (field.isFocused()) {
            Fonts.getFont("rb-r").drawString("|", (double)(field.xPosition + 4) + this.spacing(field), field.yPosition + 6, -1);
        }
        if (field.getText().equals("")) {
            Fonts.getFont("rb-r").drawString("Search...", field.xPosition + 4, field.yPosition + 6, -5592406);
        } else {
            int color = -5592406;
            if (field.isFocused()) {
                color = -1;
