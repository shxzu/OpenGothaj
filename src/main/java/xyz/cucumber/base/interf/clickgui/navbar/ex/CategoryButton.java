...
extends NavbarButtons {
    private Category category;
    private List<ModuleButton> modules = new ArrayList<ModuleButton>();
    ...
    public void draw(int mouseX, int mouseY) {
        ...
        RenderUtils.drawImage(this.position.getX() + 8.0, this.position.getY() + 2.0, this.position.getHeight() - 4.0, this.position.getHeight() - 4.0, new ResourceLocation("client/images/" + this.category.name().toLowerCase() + ".png"), color);
        Fonts.getFont("rb-r").drawString(String.valueOf(this.category.name().substring(0, 1).toUpperCase()) + this.category.name().substring(1).toLowerCase(), this.position.getX() + 30.0, this.position.getY() + this.position.getHeight() / 2.0 - (double)(Fonts.getFont("rb-r").getHeight(String.valueOf(this.category.name().substring(0, 1).toUpperCase()) + this.category.name().substring(1).toLowerCase()) / 2.0f), color);
    }
