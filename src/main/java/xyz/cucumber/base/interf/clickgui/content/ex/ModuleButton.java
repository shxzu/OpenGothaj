...
public class ModuleButton {
    ...
    private List<SettingsButton> settings = new ArrayList<SettingsButton>();
    ...
    public ModuleButton(Mod module, PositionUtils position) {
        ...
        for (ModuleSettings setting : module.getSettings()) {
            ...
            if (setting instanceof ColorSettings) {
                this.settings.add(new ColorButton((ColorSettings)setting, new PositionUtils(0.0, 0.0, position.getWidth(), 15.0, 1.0f), new PositionUtils(0.0, 0.0, 65.0, 40.0, 1.0f), new PositionUtils(0.0, 0.0, 65.0, 40.0, 1.0f), new PositionUtils(0.0, 0.0, 135.0, 4.0, 1.0f), new PositionUtils(0.0, 0.0, 80.0, 15.0, 1.0f)));
            }
            if (!(setting instanceof NumberSettings)) continue;
    ...
    public void draw(int mouseX, int mouseY) {
        ...
        RenderUtils.drawRoundedRectWithCorners(this.position.getX(), this.position.getY(), this.position.getX2(), this.position.getY2(), -1876942816, 4.0, true, true, true, true);
        double h = 0.0;
        for (SettingsButton s : this.getVisibleSettings()) {
            if (s.getSettingMain() != null && !s.getSettingMain().getVisibility().get().booleanValue()) continue;
    ...
    public double getSettingsHeight() {
        ...
        for (SettingsButton s : this.settings) {
            if (!this.open && !(s instanceof ModuleToggleButton) || s.getSettingMain() != null && !s.getSettingMain().getVisibility().get().booleanValue()) continue;
            h += s.getPosition().getHeight();
        }
        return h;
    }
    ...
    public List<SettingsButton> getVisibleSettings() {
        ArrayList<SettingsButton> set = new ArrayList<SettingsButton>();
        for (SettingsButton s : this.settings) {
            if (!this.open && !(s instanceof ModuleToggleButton) || s.getSettingMain() != null && !s.getSettingMain().getVisibility().get().booleanValue() || !(this.position.getY() <= s.getPosition().getY2()) && !(this.position.getY2() >= s.getPosition().getY())) continue;
            set.add(s);
        }
        return set;
    }
    ...
    public void clicked(int mouseX, int mouseY, int button) {
        if (this.position.isInside(mouseX, mouseY)) {
            for (SettingsButton s : this.getVisibleSettings()) {
                if (!this.open && !(s instanceof ModuleToggleButton)) continue;
