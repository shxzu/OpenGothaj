...
public class Navbar {
    ...
    public List<CategoryButton> categories = new ArrayList<CategoryButton>();
    public List<ClientButtons> client = new ArrayList<ClientButtons>();
    ...
    public void draw(int mouseX, int mouseY) {
        ...
        RenderUtils.drawImage(this.position.getX() + 14.0 - 9.0, this.position.getY() + 12.0 - 8.0, 18.0, 18.0, new ResourceLocation("client/images/gothaj.png"), -1);
        Fonts.getFont("rb-b").drawString("GOTHAJ", this.position.getX() + 14.0 + 18.0 + 5.0, this.position.getY() + 25.0 - Fonts.getFont("rb-b").getWidth("GOTHAJ") / 2.0, -12424715);
        double i = 15.0;
    ...
    public void clicked(int mouseX, int mouseY, int button) {
        if (this.position.isInside(mouseX, mouseY)) {
            for (CategoryButton categoryButton : this.categories) {
                categoryButton.clicked(mouseX, mouseY, button);
            }
            for (NavbarButtons navbarButtons : this.client) {
                navbarButtons.clicked(mouseX, mouseY, button);
            }
        }
    }
