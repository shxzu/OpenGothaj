...
public class NumberClientSetting
extends ClientSetting {
    ...
    public void draw(int mouseX, int mouseY) {
        ...
        if (this.dragging) {
            double newValue = this.roundToPlace(diff / this.slider.getWidth() * (this.setting.getMax() - this.setting.getMin()) + this.setting.getMin(), (int)this.slider.getWidth());
            this.setting.setValue((float)newValue);
        }
        Fonts.getFont("rb-m").drawString(this.setting.getName(), this.position.getX() + 8.0, this.position.getY() + 4.0, -1);
        Fonts.getFont("rb-r").drawString(String.valueOf(this.setting.getValue()), this.position.getX2() - 8.0 - Fonts.getFont("rb-r").getWidth(String.valueOf(this.setting.getValue())), this.position.getY() + 4.0, -1);
        ...
        this.animation = this.dragging ? (this.animation * 9.0 + 3.5) / 10.0 : (this.animation * 9.0 + 0.2) / 10.0;
        this.point.setX(this.position.getX() + 7.0 + (this.anim - 1.5));
        this.point.setY(this.position.getY2() - 8.0);
        RenderUtils.drawCircle(this.point.getX() + 1.5, this.point.getY() + 1.5, this.animation, 0x25FFFFFF, 2.0);
        RenderUtils.drawCircle(this.point.getX() + 1.5, this.point.getY() + 1.5, this.animation + 1.0, 0x25FFFFFF, 2.0);
        RenderUtils.drawCircle(this.point.getX() + 1.5, this.point.getY() + 1.5, this.animation + 2.0, 0x25FFFFFF, 2.0);
