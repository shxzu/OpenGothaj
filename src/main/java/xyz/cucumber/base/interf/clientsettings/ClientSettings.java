...
public class ClientSettings {
    ...
    public void draw(int mouseX, int mouseY) {
        ...
        double anim = 0.0;
        anim = this.open ? (double)this.animation.getAnimation(1.0f, 10) : (double)(1.0f - this.animation.getAnimation(1.0f, 10));
        if (anim != 0.0) {
            BlurUtils.renderBlur(5.0f);
            GL11.glPushMatrix();
            GL11.glTranslated((double)(this.position.getX() - this.position.getX() * anim + this.position.getWidth() / 2.0 - this.position.getWidth() / 2.0 * anim), (double)(this.position.getY() - this.position.getY() * anim + this.position.getHeight() / 2.0 - this.position.getHeight() / 2.0 * anim), (double)0.0);
            GL11.glScaled((double)anim, (double)anim, (double)1.0);
