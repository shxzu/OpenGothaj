...
public class NewCategoryButton
extends Button {
    public List<NewModuleButton> modules = new ArrayList<NewModuleButton>();
    ...
    public void draw(int mouseX, int mouseY) {
        this.animation = this.isActive || this.position.isInside(mouseX, mouseY) ? (this.animation * 9.0 + 1.0) / 10.0 : this.animation * 9.0 / 10.0;
        ...
        Fonts.getFont("mitr").drawString(String.valueOf(this.category.name().toLowerCase().substring(0, 1).toUpperCase()) + this.category.name().toLowerCase().substring(1), this.position.getX() + 35.0, this.position.getY() + 7.0, color);
    }
