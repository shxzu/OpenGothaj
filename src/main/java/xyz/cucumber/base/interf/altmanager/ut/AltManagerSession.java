...
public class AltManagerSession {
    ...
    public void draw(int mouseX, int mouseY) {
        ...
        while (n2 < n) {
            ...
            Fonts.getFont("rb-b").drawStringWithShadow(t, this.position.getX() + 3.0 + w, this.position.getY() + 6.0, ColorUtils.mix(-10007340, -12751688, Math.sin(Math.toRadians((double)(System.nanoTime() / 1000000L) + w * 10.0) / 3.0) + 1.0, 2.0), 0x45000000);
            w += Fonts.getFont("rb-b").getWidth(t);
            ++n2;
        }
        Fonts.getFont("rb-r-13").drawString(this.session.getToken().equals("0") ? "Cracked" : "Online", this.position.getX() + this.position.getWidth() / 2.0 - Fonts.getFont("rb-r").getWidth(this.session.getToken().equals("0") ? "Cracked" : "Online") / 2.0, this.position.getY() + 6.0, -8947849);
