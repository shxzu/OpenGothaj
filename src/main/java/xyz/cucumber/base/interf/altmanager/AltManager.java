...
extends GuiScreen {
    ...
    public void initGui() {
        this.buttons.clear();
        this.sessions.clear();
        this.position = new PositionUtils(130.0, 2.0, this.width - 6 - 130, this.height - 4, 1.0f);
        ...
        this.buttons.add(new AltManagerClickable(1, "Add", 15.0, this.height - 70, 100.0, 15.0));
        this.buttons.add(new AltManagerClickable(2, "Remove", 15.0, this.height - 53, 100.0, 15.0));
        this.buttons.add(new AltManagerClickable(3, "Random cracked", 15.0, this.height - 87, 100.0, 15.0));
        this.buttons.add(new AltManagerClickable(5, "Legit cracked", 15.0, this.height - 104, 100.0, 15.0));
        this.buttons.add(new AltManagerClickable(4, "Cookie from Clipboard", 15.0, this.height - 121, 100.0, 15.0));
        this.buttons.add(new AltManagerClickable(6, "Cookie Login (Best Version)", 15.0, this.height - 138, 100.0, 15.0));
    }
    ...
    public void drawScreen(int mouseX, int mouseY, float partialTicks) {
        ...
        RenderUtils.drawRoundedRect(this.position.getX(), this.position.getY(), this.position.getX2(), this.position.getY2(), 0x30000000, 3.0);
        int x = 0;
        double y = 0.0;
        for (AltManagerSession session : this.sessions) {
        ...
        Fonts.getFont("rb-b").drawString("Alt Manager", 65.0 - Fonts.getFont("rb-b").getWidth("Alt Manager") / 2.0, 25.0 - Fonts.getFont("rb-b").getWidth("GOTHAJ") / 2.0, -3355444);
        RenderUtils.drawRoundedRect(2.0, 25.0, 128.0, 63.0, 0x30000000, 2.0f);
        ...
        while (n2 < n) {
            ...
            Fonts.getFont("rb-b").drawStringWithShadow(t, 65.0 - Fonts.getFont("rb-b").getWidth("Active Session") / 2.0 + w, 31.0, ColorUtils.mix(-10007340, -12751688, Math.sin(Math.toRadians((double)(System.nanoTime() / 1000000L) + w * 10.0) / 3.0) + 1.0, 2.0), 0x45000000);
    ...
    protected void mouseClicked(int mouseX, int mouseY, int mouseButton) throws IOException {
        ...
        for (AltManagerSession session : this.sessions) {
            if (!session.getPosition().isInside(mouseX, mouseY) || mouseButton != 0) continue;
            session.onClick(mouseX, mouseY, mouseButton);
        }
        for (Button b : this.buttons) {
            if (!b.getPosition().isInside(mouseX, mouseY) || mouseButton != 0) continue;
            switch (b.getId()) {
                case 0: {
