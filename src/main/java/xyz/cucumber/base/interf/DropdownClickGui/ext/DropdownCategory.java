...
import xyz.cucumber.base.interf.DropdownClickGui.ext.DropdownModule;
...
implements DropdownButton {
    ...
    public void draw(int mouseX, int mouseY) {
        ...
        RenderUtils.drawOutlinedRoundedRect(this.position.getX(), this.position.getY(), this.position.getX2(), this.position.getY2(), 0x30000000, 4.0, 2.0);
        RenderUtils.drawRoundedRect(this.position.getX(), this.position.getY(), this.position.getX2(), this.position.getY2(), -300410856, 4.0f);
        RenderUtils.drawImage(this.position.getX() + 3.0, this.position.getY() + 3.0, 15.0, 15.0, new ResourceLocation("client/images/" + this.category.name().toLowerCase() + ".png"), -1);
        Fonts.getFont("rb-b").drawString(this.category.name(), this.position.getX() + this.position.getWidth() / 2.0 - Fonts.getFont("rb-b").getWidth(this.category.name()) / 2.0, this.position.getY() + 8.0, -1);
        double h = 0.0;
        if (this.open || this.position.getHeight() >= 16.0) {
            RenderUtils.enableScisor();
            RenderUtils.scissor(new ScaledResolution(Minecraft.getMinecraft()), this.position.getX() + 5.0, this.position.getY() + 20.0, 100.0, this.position.getHeight() - 21.0);
    ...
    public void onClick(int mouseX, int mouseY, int button) {
        if (this.position.isInside(mouseX, mouseY)) {
            if ((double)mouseX > this.position.getX() && this.position.getX2() > (double)mouseX && (double)mouseY > this.position.getY() + 15.0 && (double)mouseY < this.position.getY2()) {
                for (DropdownButton b : this.buttons) {
                    DropdownModule but = (DropdownModule)b;
                    if (!but.getPosition().isInside(mouseX, mouseY)) continue;
