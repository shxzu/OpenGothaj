...
implements DropdownButton {
    ...
    public DropdownModule(Mod module) {
        ...
        for (ModuleSettings s : module.getSettings()) {
            ...
            if (!(s instanceof NumberSettings)) continue;
    ...
    public void draw(int mouseX, int mouseY) {
        ...
        int color = -297323283;
        this.Coloranimation = this.module.isEnabled() ? (this.Coloranimation * 10.0 + 100.0) / 11.0 : (this.Coloranimation < 10.0 ? 0.0 : this.Coloranimation - 10.0);
        ...
        Fonts.getFont("rb-m").drawString(this.module.getName(), this.position.getX() + 3.0, this.position.getY() + 6.0, ColorUtils.mix(-5592406, color, this.Coloranimation, 100.0));
        ...
        if (this.open || this.position.getHeight() >= this.defaultHeight + 1.0) {
            ...
            for (DropdownClickGuiSettings set : this.settings) {
                if (set.getMainSetting() != null && !set.getMainSetting().getVisibility().get().booleanValue()) continue;
    ...
    public void onClick(int mouseX, int mouseY, int button) {
        if (this.position.isInside(mouseX, mouseY)) {
            for (DropdownClickGuiSettings set : this.settings) {
                if (set.getMainSetting() != null && !set.getMainSetting().getVisibility().get().booleanValue() || !set.getPosition().isInside(mouseX, mouseY)) continue;
    ...
    public void renderArrow(double x, double y, double size, int color, double angle) {
        ...
        GL11.glVertex2d((double)(x + size / 3.0), (double)(y - size / 3.0));
        GL11.glVertex2d((double)x, (double)(y + size / 3.0));
        GL11.glEnd();
        RenderUtils.stop2D();
