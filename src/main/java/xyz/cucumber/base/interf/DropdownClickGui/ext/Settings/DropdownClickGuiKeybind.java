...
public class DropdownClickGuiKeybind
extends DropdownClickGuiSettings {
    ...
    public void draw(int mouseX, int mouseY) {
        String s = Keyboard.getKeyName((int)this.module.getKey());
        if (this.binding) {
            s = "...";
        }
        Fonts.getFont("rb-m-13").drawString("Key: " + s, this.position.getX() + this.position.getWidth() / 2.0 - Fonts.getFont("rb-m-13").getWidth("Key: " + s) / 2.0, this.position.getY() + 2.0, -1);
    }
