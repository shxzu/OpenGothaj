...
public class MinecraftAuth {
    ...
    public static String getMinecraftBearerToken(String userHash, String xSTSToken) throws IOException, MinecraftException {
        ...
        requestBody.addProperty("ensureLegacyEnabled", Boolean.valueOf(true));
        Connection.Response response = null;
        try {
            response = Jsoup.connect(MINECRAFT_AUTH_URL).method(Connection.Method.POST).requestBody(requestBody.toString()).header("Content-Type", "application/json").header("Accept", "application/json").ignoreContentType(true).execute();
        }
    ...
    public static MinecraftProfileResponse getMinecraftProfile(String bearerToken) throws IOException, MinecraftException {
        Connection.Response response = Jsoup.connect(MINECRAFT_PROFILE_URL).method(Connection.Method.GET).header("Authorization", "Bearer " + bearerToken).ignoreContentType(true).ignoreHttpErrors(true).execute();
