...
public class MicrosoftAuth {
    ...
    public static AccessTokenResponse getAccessTokenWithCredentials(String email, String password) throws MicrosoftException, IOException {
        AuthorizeRequest authorizeRequest = MicrosoftAuth.getAuthorizeRequest();
        String requestBody = "login=" + URLEncoder.encode(email, "UTF-8") + "&loginfmt=" + URLEncoder.encode(email, "UTF-8") + "&passwd=" + URLEncoder.encode(password, "UTF-8") + "&PPFT=" + authorizeRequest.getSFTTag();
        Connection.Response response = Jsoup.connect(authorizeRequest.getUrlPost()).method(Connection.Method.POST).cookies(authorizeRequest.getCookies()).requestBody(requestBody).header("Content-Type", "application/x-www-form-urlencoded").followRedirects(true).execute();
        String responseBody = response.body();
        if (responseBody.contains("Sign in to")) {
            throw new MicrosoftException("Invalid credentials provided");
        }
        if (responseBody.contains("Help us protect your account")) {
    ...
    public AccessTokenResponse getAccessToken(String code) throws IOException, MicrosoftException {
        ...
        try {
            response = Jsoup.connect(LIVE_TOKEN_URL).method(Connection.Method.POST).data("client_id", this.appID).data("client_secret", this.appSecret).data("redirect_uri", this.redirectUri).data("grant_type", "authorization_code").data("code", code).ignoreContentType(true).execute();
        }
    ...
    public AccessTokenResponse refreshAccessToken(String rToken) throws IOException, MicrosoftException {
        Connection.Response response = null;
        try {
            response = Jsoup.connect(LIVE_TOKEN_URL).method(Connection.Method.POST).data("client_id", this.appID).data("client_secret", this.appSecret).data("refresh_token", rToken).data("grant_type", "refresh_token").data("redirect_uri", this.redirectUri).ignoreContentType(true).execute();
        }
        catch (HttpStatusException e) {
    ...
    public static XboxLiveAuthResponse getXboxLiveAuthResponse(String accessToken, boolean oAuth2) throws IOException, MicrosoftException {
        ...
        Connection.Response response = null;
        try {
            response = Jsoup.connect(XBOX_LIVE_AUTH_URL).method(Connection.Method.POST).requestBody(requestBody.toString()).header("Content-Type", "application/json").header("Accept", "application/json").ignoreContentType(true).ignoreHttpErrors(true).execute();
        }
        catch (HttpStatusException e) {
            throw new MicrosoftException("Invalid access token provided");
        }
        JsonObject jsonResponse = (JsonObject)gson.fromJson(response.body(), JsonObject.class);
        return new XboxLiveAuthResponse(jsonResponse.get("Token").getAsString(), jsonResponse.get("DisplayClaims").getAsJsonObject().get("xui").getAsJsonArray().get(0).getAsJsonObject().get("uhs").getAsString());
    }
    ...
    public static XSTSResponse getXSTSResponse(String token) throws IOException, MicrosoftException {
        ...
        userTokens.add((JsonElement)new JsonPrimitive(token));
        properties.add("UserTokens", (JsonElement)userTokens);
        requestBody.add("Properties", (JsonElement)properties);
        Connection.Response response = null;
        response = Jsoup.connect(XBOX_XSTS_AUTH_URL).method(Connection.Method.POST).requestBody(requestBody.toString()).header("Content-Type", "application/json").header("Accept", "application/json").ignoreContentType(true).ignoreHttpErrors(true).execute();
        JsonObject jsonResponse = (JsonObject)gson.fromJson(response.body(), JsonObject.class);
        if (jsonResponse.has("XErr")) {
            switch (jsonResponse.get("XErr").getAsString()) {
                case "2148916233": {
        ...
        return new XSTSResponse(jsonResponse.get("Token").getAsString(), jsonResponse.get("DisplayClaims").getAsJsonObject().get("xui").getAsJsonArray().get(0).getAsJsonObject().get("uhs").getAsString());
    }
