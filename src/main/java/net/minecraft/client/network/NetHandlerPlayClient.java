...
implements INetHandlerPlayClient {
    ...
    public void handleMultiBlockChange(S22PacketMultiBlockChange packetIn) {
        PacketThreadUtil.checkThreadAndEnqueue(packetIn, this, this.gameController);
        S22PacketMultiBlockChange.BlockUpdateData[] blockUpdateDataArray = packetIn.getChangedBlocks();
        int n = blockUpdateDataArray.length;
        int n2 = 0;
        while (n2 < n) {
            S22PacketMultiBlockChange.BlockUpdateData s22packetmultiblockchange$blockupdatedata = blockUpdateDataArray[n2];
    ...
    public void handleMapChunkBulk(S26PacketMapChunkBulk packetIn) {
        PacketThreadUtil.checkThreadAndEnqueue(packetIn, this, this.gameController);
        int i = 0;
        while (i < packetIn.getChunkCount()) {
    ...
    public void handleCustomPayload(S3FPacketCustomPayload packetIn) {
        block12: {
            try {
                ...
                } else if ("MC|BOpen".equals(packetIn.getChannelName()) && (itemstack = this.gameController.thePlayer.getCurrentEquippedItem()) != null && itemstack.getItem() == Items.written_book) {
    ...
    public void handleParticles(S2APacketParticles packetIn) {
        ...
        if (packetIn.getParticleCount() == 0) {
            ...
            int i = 0;
            while (i < packetIn.getParticleCount()) {
