...
extends GuiScreen {
    ...
    public void initGui() {
        ...
        double xPosition = -10000.0;
        double yPosition = -10000.0;
    }
    ...
    protected void mouseClicked(int mouseX, int mouseY, int mouseButton) throws IOException {
        ...
        for (Mod m : Client.INSTANCE.getModuleManager().getDraggable()) {
            if (!m.isEnabled() || m.getPosition() == null || !m.getPosition().isInside(mouseX, mouseY)) continue;
            this.draggingModule = m;
            this.dragging = true;
            double[] pos = PositionHandler.getPositionFromValue(m.getPosition().getX(), m.getPosition().getY());
            this.dragX = (double)mouseX - pos[0];
            this.dragY = (double)mouseY - pos[1];
            return;
        }
    ...
    public void drawScreen(int mouseX, int mouseY, float partialTicks) {
        ...
        if (this.dragging) {
            double x = (double)mouseX - this.dragX;
            double y = (double)mouseY - this.dragY;
            double[] pos = PositionHandler.getPositionFromValue(x, y);
            this.draggingModule.setXYPosition(x, y);
        }
    ...
    public void onAutocompleteResponse(String[] p_146406_1_) {
        if (this.waitingOnAutocomplete) {
            this.playerNamesFound = false;
            this.foundPlayerNames.clear();
            String[] stringArray = p_146406_1_;
            int n = p_146406_1_.length;
            int n2 = 0;
            while (n2 < n) {
