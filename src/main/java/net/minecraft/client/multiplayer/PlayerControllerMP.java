...
public class PlayerControllerMP {
    ...
    public boolean onPlayerDamageBlock(BlockPos posBlock, EnumFacing directionFacing) {
        ...
        if (this.isHittingPosition(posBlock)) {
            ...
            Client.INSTANCE.getEventBus().call(event);
            curBlockDamageMP += event.getPlayerRelativeBlockHardness();
    ...
    public void attackEntityEvent(EntityPlayer playerIn, Entity targetEntity) {
        ...
        if (event.isForced()) {
            ...
            if (i != this.currentPlayerItem) {
                this.currentPlayerItem = i;
                this.mc.getNetHandler().getNetworkManager().sendPacketNoEvent(new C09PacketHeldItemChange(this.currentPlayerItem));
            }
            this.mc.getNetHandler().getNetworkManager().sendPacketNoEvent(new C02PacketUseEntity(targetEntity, C02PacketUseEntity.Action.ATTACK));
        } else {
            this.syncCurrentPlayItem();
            this.netClientHandler.addToSendQueue(new C02PacketUseEntity(targetEntity, C02PacketUseEntity.Action.ATTACK));
        }
