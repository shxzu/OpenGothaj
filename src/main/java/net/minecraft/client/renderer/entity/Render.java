...
implements IEntityRenderer {
    ...
    public void doRender(T entity, double x, double y, double z, float entityYaw, float partialTicks) {
        ...
        if (e.isCancelled() || !ShaderESPModule.nametags) {
            return;
        }
    ...
    protected void renderName(T entity, double x, double y, double z) {
        if (this.canRenderName(entity) && ShaderESPModule.nametags) {
            this.renderLivingLabel(entity, ((Entity)entity).getDisplayName().getFormattedText(), x, y, z, 64);
        }
    }

    protected boolean canRenderName(T entity) {
        return ((Entity)entity).getAlwaysRenderNameTagForRender() && ((Entity)entity).hasCustomName() && ShaderESPModule.nametags;
    }
    ...
    private void renderShadow(Entity entityIn, double x, double y, double z, float shadowAlpha, float partialTicks) {
        if (Client.INSTANCE.getModuleManager().getModule(BoosterModule.class).isEnabled()) {
            return;
        }
